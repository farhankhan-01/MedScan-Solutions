...
class PatientBloc extends ChangeNotifier {
  ...
  Future<void> updateMedicines() async {
    // final medicinesRef = _firestore.collection('Prescriptions');
    // final box = Hive.box(MEDICINES_BOX);
    // medicinesRef.get().then((QuerySnapshot docs) {
  ...
  Future<void> scanPrescription(String imagePath) async {
    ...
    const path = 'captures/test_app';

    final UploadTask uploadTask =
        _storageReference.child(path).putFile(File(imagePath));

    await uploadTask.whenComplete(() {}).then((snapshot) async {
    ...
    var response =
        await http.post(Uri.parse(url), body: {'imageURL': imageURL});
    ...
    if (response.statusCode == 500) {
      ...
      predictions.forEach((element) async {
        QuerySnapshot snapshots = await _firestore
            ...
            .get();
        final med = snapshots.docs[0].data as Map<String, dynamic>;
        medicines = [
          ...medicines,
          Medicine(
